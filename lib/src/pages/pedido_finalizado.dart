...
class PedidosFinalizados extends StatefulWidget {
  const PedidosFinalizados({super.key});

  @override
  State<PedidosFinalizados> createState() => _PedidosFinalizadosState();
}
...
class _PedidosFinalizadosState extends State<PedidosFinalizados> {
        ...
        if (snapshot.hasData) {
          ...
          return FutureBuilder<List<PedidoModel>>(
            ...
            builder: (BuildContext context,
                AsyncSnapshot<List<PedidoModel>> pedidosSnapshot) {
  ...
  Future<List<PedidoModel>> _obtenerPedidosDeUsuarios(
    ...
    for (var usuarioDoc in usuarios) {
      ...
      if (ordenesSnapshot.docs.isNotEmpty) {
        for (var orden in ordenesSnapshot.docs) {
          if (orden['estado'] == 'OrderStatus.finished') {
     ...
     // Ordenar la lista de pedidos por el nombre del cliente
    pedidos.sort((a, b) => a.nombre.compareTo(b.nombre));
